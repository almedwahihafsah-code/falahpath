...
const FEATURE_ICONS = [Bell, BookOpen, Compass];
...
export const FalahiTeaser = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!parsed.success) {
      toast.error(f.errInvalid);
      return;
    }
    setSubmitting(true);
    const { error } = await supabase
      .from("newsletter_subscribers")
      .insert({ email: parsed.data.toLowerCase(), source: "falahi_waitlist", locale: lang });
    setSubmitting(false);

    // Treat duplicates (23505) the same as success
    if (error && error.code !== "23505") {
      toast.error(f.errSave);
      return;
    }
    setDone(true);
    ...
    >
      ...
      <div className="container relative text-center">
        ...
        <h2 className="font-editorial text-5xl sm:text-6xl md:text-7xl text-primary-foreground leading-[1] tracking-tight mb-4">
          <em className="not-italic italic text-gradient-gold">{f.title}</em>
        </h2>
        ...
        >
          <h3 className="font-editorial text-2xl md:text-3xl text-primary-foreground mb-3">
            {f.waitlistTitleLead} <em className="not-italic italic text-accent">{f.waitlistTitleEm}</em>
          </h3>
